        ...
        <card>
          <template slot="header">
            <div class="row justify-content-between align-items-between mx-3">
              ...
              <h5 class="card-title">Power Usage Reports</h5>
              
              <div>
                  <base-button class="btn btn-primary" @click="modalPowerUsageReports = true">Add</base-button>
              </div>
              
              <modal :show.sync="modalPowerUsageReports">
                <template slot="header">
                  <h5 class="modal-title" id="exampleModalLabel">Add PowerUsageReport</h5>
                </template>
                <div>
                  <form @submit.prevent>
  <base-input label="PowerUsageReportId" type="text" placeholder="Enter PowerUsageReportId" v-model="powerUsageReportToAdd.powerUsageReportId"></base-input>
  <base-input label="TotalPowerUsage" type="text" placeholder="Enter TotalPowerUsage" v-model="powerUsageReportToAdd.totalPowerUsage"></base-input>
  <base-input label="ReportDate" type="text" placeholder="Enter ReportDate" v-model="powerUsageReportToAdd.reportDate"></base-input>
                  </form>
                </div>
          ...
          <div v-if="isTableView">
            ...
            <a-table :columns="columns" :data-source="powerUsageReports" :row-key="record => record.PowerUsageReportId" :pagination="pagination"
          ...
          <div v-else>
            ...
            <PowerUsageReportPictureView :powerUsageReports="powerUsageReports" />
          </div>
...
import PowerUsageReportPictureView from './PowerUsageReportPictureView.vue';


const powerUsageReportsColumns = [
  "powerUsageReportId",
  "year",
  "date",
  "competitionId",
...
export default {
  ...
  components: {
    ...
    Pagination,
    PowerUsageReportPictureView
  },

  data() {
    return {
      modalPowerUsageReports: false,
        isTableView: true,

      columns: [
        {
          title: 'Power Usage Report Id',
		dataIndex: 'powerUsageReportId',
          visible: true,
          scopedSlots: { customRender: 'powerUsageReportId' },
          sorter: true
          //sorter: (a, b) => a.powerUsageReportId - b.powerUsageReportId,
          //sorter: (a, b) => a.powerUsageReportId.localeCompare(b.powerUsageReportId),
        },
        {
          title: 'Total Power Usage',
		dataIndex: 'totalPowerUsage',
          visible: true,
          scopedSlots: { customRender: 'totalPowerUsage' },
          sorter: true
          //sorter: (a, b) => a.totalPowerUsage - b.totalPowerUsage,
          //sorter: (a, b) => a.totalPowerUsage.localeCompare(b.totalPowerUsage),
        },
        {
          title: 'Report Date',
      ...
      pagination: {
        ...
        showTotal: (total) => `Total ${total} powerUsageReports`,
      },

      powerUsageReports: [],
      powerUsageReportToAdd: {},

      powerUsageReportsTable: {
        columns: [...powerUsageReportsColumns],
        data: [],
      },

      // New properties for sorting and searching
      sortBy: 'powerUsageReportId',           // Column to sort by
  ...
  methods: {
    ...
    async renderPowerUsageReportsTable() {
      ...
      let powerUsageReportsTableData = [];
      for (let i = 0; i < this.powerUsageReports.length; i++) {
        powerUsageReportsTableData.push({
          id: i,
          powerUsageReportId: this.powerUsageReports[i].powerUsageReportId,
          year: this.powerUsageReports[i].year,
          date: this.powerUsageReports[i].date,
          competitionId: this.powerUsageReports[i].competitionId,
          powerUsageReportId: this.powerUsageReports[i].powerUsageReportId,
        });
    ...
    async onTableChange(pagination, filters, sorter) {
	  ...
	  this.$emit('get-all-power-usage-reports',this.sortBy,this.sortOrder,this.pagination.current-1,this.pagination.pageSize);
    ...
    handleSearchQueryChanged() {
    	...
    	this.$root.$emit('searchQueryForPowerUsageReportsChanged', this.searchQuery);
		//this.renderPowerUsageReportsTable();
    },
	...
	async handleAddSubmitted() {
      this.modalPowerUsageReports = false;

      const currentDate = new Date().getTime();
      this.powerUsageReportToAdd.created = currentDate;

      const jsonData = JSON.stringify(this.powerUsageReportToAdd);
      console.log(jsonData);
      
      const res = await PowerUsageReportService.addPowerUsageReport(jsonData);
