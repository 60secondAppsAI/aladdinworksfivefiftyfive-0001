        ...
        <card>
          <template slot="header">
            <div class="row justify-content-between align-items-between mx-3">
              ...
              <h5 class="card-title">Light Switchs</h5>
              
              <div>
                  <base-button class="btn btn-primary" @click="modalLightSwitchs = true">Add</base-button>
              </div>
              
              <modal :show.sync="modalLightSwitchs">
                <template slot="header">
                  <h5 class="modal-title" id="exampleModalLabel">Add LightSwitch</h5>
                </template>
                <div>
                  <form @submit.prevent>
  <base-input label="LightSwitchId" type="text" placeholder="Enter LightSwitchId" v-model="lightSwitchToAdd.lightSwitchId"></base-input>
  <base-input label="Status" type="text" placeholder="Enter Status" v-model="lightSwitchToAdd.status"></base-input>
                  </form>
                </div>
          ...
          <div v-if="isTableView">
            ...
            <a-table :columns="columns" :data-source="lightSwitchs" :row-key="record => record.LightSwitchId" :pagination="pagination"
          ...
          <div v-else>
            ...
            <LightSwitchPictureView :lightSwitchs="lightSwitchs" />
          </div>
...
import LightSwitchPictureView from './LightSwitchPictureView.vue';


const lightSwitchsColumns = [
  "lightSwitchId",
  "year",
  "date",
  "competitionId",
...
export default {
  ...
  components: {
    ...
    Pagination,
    LightSwitchPictureView
  },

  data() {
    return {
      modalLightSwitchs: false,
        isTableView: true,

      columns: [
        {
          title: 'Light Switch Id',
		dataIndex: 'lightSwitchId',
          visible: true,
          scopedSlots: { customRender: 'lightSwitchId' },
          sorter: true
          //sorter: (a, b) => a.lightSwitchId - b.lightSwitchId,
          //sorter: (a, b) => a.lightSwitchId.localeCompare(b.lightSwitchId),
        },
        {
          title: 'Status',
		dataIndex: 'status',
      ...
      pagination: {
        ...
        showTotal: (total) => `Total ${total} lightSwitchs`,
      },
      ...
      lightSwitchsTable: {
        columns: [...lightSwitchsColumns],
        data: [],
      },

      // New properties for sorting and searching
      sortBy: 'lightSwitchId',           // Column to sort by
  ...
  methods: {
    ...
    async renderLightSwitchsTable() {
      ...
      let lightSwitchsTableData = [];
      for (let i = 0; i < this.lightSwitchs.length; i++) {
        lightSwitchsTableData.push({
          id: i,
          lightSwitchId: this.lightSwitchs[i].lightSwitchId,
          year: this.lightSwitchs[i].year,
          date: this.lightSwitchs[i].date,
          competitionId: this.lightSwitchs[i].competitionId,
          lightSwitchId: this.lightSwitchs[i].lightSwitchId,
        });
    ...
    async onTableChange(pagination, filters, sorter) {
	  ...
	  this.$emit('get-all-light-switchs',this.sortBy,this.sortOrder,this.pagination.current-1,this.pagination.pageSize);
    ...
    handleSearchQueryChanged() {
    	...
    	this.$root.$emit('searchQueryForLightSwitchsChanged', this.searchQuery);
		//this.renderLightSwitchsTable();
    },
	...
	async handleAddSubmitted() {
      this.modalLightSwitchs = false;

      const currentDate = new Date().getTime();
      this.lightSwitchToAdd.created = currentDate;

      const jsonData = JSON.stringify(this.lightSwitchToAdd);
      console.log(jsonData);
      
      const res = await LightSwitchService.addLightSwitch(jsonData);
